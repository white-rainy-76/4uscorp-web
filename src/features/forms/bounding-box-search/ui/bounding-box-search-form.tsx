...
export interface BoundingBoxSearchFormProps {
  title?: string
  description?: string
  resultsCount?: number
  resultsLabel?: string
  ...
  onSearch: (coordinates: BoundingBoxCoordinates) => void
  defaultCoordinates?: {
    minLat: string
    minLon: string
    maxLat: string
...
export const BoundingBoxSearchForm = ({
  title = 'Search',
  description,
  resultsCount,
  resultsLabel = 'Found',
  ...
  const {
    ...
    setValue,
    reset,
  } = useForm<BoundingBoxSearchFormValues>({
    resolver: zodResolver(boundingBoxSearchFormSchema),
    mode: 'onBlur', // Валидация только при blur и submit
    defaultValues: {
      minLat: defaultCoordinates.minLat,
      minLon: defaultCoordinates.minLon,
      maxLat: defaultCoordinates.maxLat,
      maxLon: defaultCoordinates.maxLon,
    },
  })

  const onSubmit = (data: BoundingBoxSearchFormValues) => {
    onSearch({
  ...
  return (
    <div className="absolute top-4 left-4 z-10">
      <InfoCard
        title={title}
        className={`${className} shadow-xl border border-gray-200 bg-white/95 backdrop-blur-sm`}>
        {description && (
